...
const Exam = ({ exam, onExamUpdate, updateEditedState }) => {
	...
	return (
		<div className="exam flex flex-col items-start gap-1">
			...
			<div className="input-field inline-flex items-center w-[100%]">
				<input
					type="number"
					name={exam.name}
					id={exam.name}
				...
				/>
				<div
					className={`h-8 text-base font-normal text-text-500 bg-white border-y border-r rounded-r px-2 flex items-center ${
						!exam.isCalculated ? "border-secondary-200" : "border-primary-500"
					}`}>{`Coef ${exam.coef}`}</div>
			</div>
		</div>
