...
export default function LvlDropdown({ levelList, setChosenLevel }) {
	...
	return (
		<div ref={dropDownRef} className="relative">
			...
			<div
				className={`dropdown-anim mt-[-1rem] pt-4 overflow-hidden ${
					isFolded
						? "max-h-0"
						: "max-h-24 hover:overflow-auto border-x-2 border-b-2 border-primary-600 rounded-b-lg"
				}`}>
				{levelList.map((level, index) => (
					<div
						key={index}
						className="flex-col gap-4 cursor-pointer hover:bg-black/5 relative"
